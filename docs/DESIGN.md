...
### Safe Modification Strategy (M3.1)
...
**Key Functions** (`crates/mindtape-eval/src/write.rs`):
...
**Format Preservation**:
- Uses `SyntaxNode::range()` to get exact byte offsets
- Replaces only the checkbox pattern, preserving:
...
- Should we use `tempfile` for atomic writes in write-back operations?
  (Currently: using `std::fs::write` directly)
